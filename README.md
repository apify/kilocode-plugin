...
| Action | What it does |
| --- | --- |
| `discover` | Search Apify Store (`query`), **or** fetch an Actor's input schema + README (`actorId`). |
...
| `enabled` | Force on/off. When unset, the plugin enables itself if a token is present. |
